...
def send_line_push_notification(event: dict) -> bool:
    ...
    clean_title = title.replace("【LivePocket】", "").replace("【TIGET】", "").replace("【TicketDive】", "").replace("【X告知】", "").replace("【HP告知】", "").replace("【Web検索】", "").replace("【公式カレンダー】", "").replace("【TimeTree】", "").strip()
    ...
    elif source_name == "Red Radiance TimeTree":
        message_text = (
            f"【新着イベント｜TimeTree】\n"
            f"・タイトル：{clean_title}\n"
            f"・日付：{display_date}{time_display}\n"
